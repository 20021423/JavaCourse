Dưới đây là tài liệu hướng dẫn end-to-end, step-by-step chi tiết cho một dự án Spring Boot 3.x (Java 17) với các chức năng:
...
- Đăng ký người dùng
- Tài liệu hóa API với Swagger (sử dụng springdoc-openapi)
- Giao diện được render bằng Thymeleaf (sử dụng layout, meta-data, CSS, JS)

Tài liệu này được cập nhật theo Spring Boot 3.4.3 (Java 17) và bổ sung tích hợp Swagger để tài liệu hóa API.
...
- **Sản phẩm:**  
  - Thực hiện CRUD sản phẩm với các trường:  
    - `id`  
    - `name`  
    - `description`  
    - `price`  
    - `imageUrl` (URL ảnh từ dịch vụ như dummyimage.com)

- **Giỏ hàng:**  
  - Cho phép người dùng (chưa đăng nhập) thêm sản phẩm vào giỏ hàng (lưu tạm theo session) và cập nhật số lượng.

- **Đặt hàng:**  
  - Khi bấm “Thanh toán”: nếu người dùng chưa đăng nhập, sẽ yêu cầu xác thực (qua JWT hoặc chuyển giao diện đăng nhập).  
...
- **Tài liệu hóa API:**  
  - Tích hợp Swagger với springdoc-openapi để tự động tạo tài liệu OpenAPI (Swagger UI) cho các API.

### 1.2 Kiến Trúc Ứng Dụng

- **Mô hình MVC:**  
  - **Model:** Các entity (Product, User, Order, OrderItem)  
  - **View:** Giao diện HTML được render bởi Thymeleaf (sử dụng layout, meta-data, CSS, JS)  
...
- **Các Layer chính:**  
  ...
  - **Service:** Xử lý nghiệp vụ (CRUD sản phẩm, giỏ hàng, đặt hàng, đăng ký)  
  ...
  - **Payload:** Các lớp DTO (request/response) như LoginRequest, ProductRequest, RegistrationRequest, JwtResponse, RegistrationResponse (và tùy chọn ResponseObject) để định nghĩa “contract” dữ liệu giữa client và server  
...
- Truy cập [Spring Initializr](https://start.spring.io/) và chọn:  
  ...
  - **Dependencies:**  
    ...
    - *Springdoc-openapi-starter-webmvc-ui* (cho Swagger)
...
<!-- pom.xml: Cấu hình Maven cho dự án Spring Boot 3.4.3 với Java 17 -->
...
Ví dụ về cấu trúc thư mục (đã được chia module theo separation of concerns):
...
> **Chú ý:**  
> Ở Spring Boot 3.x, các annotation của JPA chuyển sang sử dụng **jakarta.persistence**.
...
## 5. Tích Hợp Swagger với Springdoc-openapi

### 5.1 Dependency
(Đã được thêm trong pom.xml với dependency:)
```xml
<dependency>
  <groupId>org.springdoc</groupId>
  <artifactId>springdoc-openapi-starter-webmvc-ui</artifactId>
...
### 5.2 Cách sử dụng
Sau khi build và chạy ứng dụng, Springdoc-openapi sẽ tự động quét các endpoint và tạo tài liệu OpenAPI. Bạn có thể truy cập giao diện Swagger UI tại:
- [http://localhost:8080/swagger-ui/index.html](http://localhost:8080/swagger-ui/index.html)

Để tùy chỉnh thêm thông tin endpoint, bạn có thể sử dụng các annotation của OpenAPI như:
```java
import io.swagger.v3.oas.annotations.Operation;

@Operation(summary = "Lấy thông tin demo", description = "Trả về thông tin demo cho kiểm tra Swagger")
@GetMapping("/demo")
...
- **Chạy ứng dụng:**  
  ...
  - Thêm sản phẩm vào giỏ hàng qua `/cart/add/{productId}`; kiểm tra giỏ hàng tại `/cart` và cập nhật số lượng.  
  ...
  - Kiểm tra luồng đặt hàng: sau khi xác thực, gọi `/order/confirm` và xem trang `/order/success`.  
  - Truy cập Swagger UI tại [http://localhost:8080/swagger-ui/index.html](http://localhost:8080/swagger-ui/index.html) để xem tài liệu API và thử nghiệm.
...
- **Build ứng dụng:**  
  ...
  ```
- **Triển khai:**  
  Chạy file jar được build hoặc deploy lên server (Heroku, AWS, v.v.).

---

## 7. Quy Trình Code & Test Liên Tục

### 7.1 Phát Triển theo Iterative Development

1. **Phân tích & thiết kế:** Xác định yêu cầu, xây dựng tài liệu thiết kế, sơ đồ kiến trúc và cấu trúc thư mục.
2. **Xây dựng Domain Model & Repository:** Code các entity và repository, sau đó unit test với cơ sở dữ liệu (có thể dùng H2 cho test).
3. **Xây dựng Service Layer:** Viết business logic (CRUD, giỏ hàng, đặt hàng, đăng ký) và unit test từng phương thức.
4. **Xây dựng Controller & API Endpoint:** Tạo các endpoint cho từng module, kiểm thử qua Postman và Swagger UI.
5. **Tích hợp bảo mật:** Cấu hình Spring Security, tích hợp JWT, test bảo mật qua request có/không token.
6. **Xây dựng giao diện với Thymeleaf:** Tạo các view, layout, CSS, JS, meta-data; test giao diện trên trình duyệt.
7. **Kiểm thử tổng thể & triển khai:** Integration test, manual test, fix bug khi cần thiết, và cuối cùng build & deploy ứng dụng.

### 7.2 Lợi Ích

- Phát hiện lỗi sớm khi code theo module nhỏ và kiểm thử liên tục.
- Giảm thiểu rủi ro khi tích hợp nhiều module.
- Dễ dàng mở rộng và bảo trì khi có yêu cầu thay đổi.
- CI/CD có thể tích hợp để tự động build, test, deploy.

---

# 8. Tổng Kết

Tài liệu hướng dẫn này đã cung cấp một quy trình phát triển ứng dụng end-to-end với Spring Boot 3.4.3 (Java 17) từ phân tích yêu cầu, thiết lập môi trường, cấu hình DB, xây dựng các module backend (entity, repository, service, controller), tích hợp bảo mật qua JWT, triển khai Swagger (springdoc-openapi) để tài liệu hóa API, đến xây dựng giao diện với Thymeleaf (bao gồm meta-data, CSS, JS).

Việc phát triển theo chu kỳ lặp và kiểm thử liên tục giúp bạn dễ dàng xác định lỗi và đảm bảo chất lượng phần mềm. Nếu có thắc mắc hay cần hỗ trợ thêm, hãy cứ hỏi nhé!